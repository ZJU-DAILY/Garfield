...
namespace smart::big_dataset_adapter {
    ...
    __device__ __forceinline__ char4 Load(const uint8_t* address) {
        return __ldg(reinterpret_cast<const char4*>(address)) ;
    }
        ...
        if(psize_block > 0 && little_seg_num_block == 0) {
            ...
            for(unsigned p_i = 0 ; p_i < psize_block; p_i ++) {
                ...
                if(p_i < psize_block - 1) {
                    ...
                    #pragma unroll
                    for(unsigned i = tid ; i < 128 ; i += blockDim.x * blockDim.y) { 
                        if(i % 64 >= TOPM || top_M_Cand[i % 64] == 0xFFFFFFFF) {
                            // 填充一个随机数
                            work_mem_unsigned[i] = 0xFFFFFFFF ;
                            work_mem_float[i] = INF_DIS ;
                            continue ;
    ...
    template<typename attrType = float>
    __global__ void select_path_contain_attr_filter_V2_without_output_single_channel_with_prefilter_batch_process_compressed_graph_T_dataset_resident_straight(const unsigned batch_size , const unsigned* batch_partition_ids , const unsigned* __restrict__ batch_all_graphs, const int8_t* __restrict__ batch_values, const half* __restrict__ query_data, 
        const float* __restrict__ scales_video_memory , const int* __restrict__ zero_points_video_memory ,
        unsigned* __restrict__ top_M_Cand_video_memory , float* __restrict__ top_M_Cand_dis_video_memory, unsigned* ent_pts , unsigned DEGREE , unsigned TOPM , unsigned DIM , const unsigned* batch_graph_start_index ,const unsigned* batch_graph_size ,
        const unsigned* __restrict__ batch_pids , const unsigned* batch_p_size ,const unsigned* batch_p_start_index ,const attrType* l_bound ,const attrType* r_bound ,const attrType* __restrict__ attrs , unsigned attr_dim , const unsigned* __restrict__ batch_global_graph , unsigned edges_num_in_global_graph,
        unsigned recycle_list_size ,const unsigned* __restrict__ global_idx ,const unsigned* __restrict__ local_idx ,const unsigned* seg_global_idx_start_idx , unsigned pnum ,unsigned K, unsigned* result_k_id  ,const unsigned* task_id_mapper , const unsigned* batch_little_seg_counts
    ...
    template<typename attrType = float , typename vecType = uint8_t>
    __global__ void select_path_contain_attr_filter_V2_without_output_single_channel_with_prefilter_batch_process_compressed_graph_T_dataset_resident_straight_u8(const unsigned batch_size , const unsigned* batch_partition_ids , const unsigned* __restrict__ batch_all_graphs, const vecType* __restrict__ batch_values, const vecType* __restrict__ query_data, 
        unsigned* __restrict__ top_M_Cand_video_memory , float* __restrict__ top_M_Cand_dis_video_memory, unsigned* ent_pts , unsigned DEGREE , unsigned TOPM , unsigned DIM , const unsigned* batch_graph_start_index ,const unsigned* batch_graph_size ,
        const unsigned* __restrict__ batch_pids , const unsigned* batch_p_size ,const unsigned* batch_p_start_index ,const attrType* l_bound ,const attrType* r_bound ,const attrType* __restrict__ attrs , unsigned attr_dim , const unsigned* __restrict__ batch_global_graph , unsigned edges_num_in_global_graph,
        unsigned recycle_list_size ,const unsigned* __restrict__ global_idx ,const unsigned* __restrict__ local_idx ,const unsigned* seg_global_idx_start_idx , unsigned pnum ,unsigned K, unsigned* result_k_id  ,const unsigned* task_id_mapper , const unsigned* batch_little_seg_counts
        ,const unsigned* global_vec_stored_pos , const unsigned batch_all_point_num , const unsigned OP_TYPE = 0) {
        ...
        **/
        ...
        // 每次四个字节的去读取查询向量, 存储方式为四个捆成一捆
        for(unsigned i = tid; i < (DIM / 4); i += blockDim.x * blockDim.y){
        // 读完4字节数据后从u8转换成__half类型
            // tmp_val_sha[i] = Load(&query_data[bid * DIM + 4 * i]);
            // char4 val = Load(&query_data[bid * DIM + 4 * i]);
            half4 val2 ; 
            val2.x = __half2(
                __float2half((float)query_data[bid * DIM + i * 4] / 100.0) ,
                __float2half((float)query_data[bid * DIM + i * 4 + 1] / 100.0)
            ) ;
            val2.y = __half2(
                __float2half((float) query_data[bid * DIM + i * 4 + 2] / 100.0) ,
                __float2half((float) query_data[bid * DIM + i * 4 + 3] / 100.0) 
            ) ;
            tmp_val_sha[i] = val2 ;
        }
        // initialize
            ...
            for(unsigned p_i = 0 ; p_i < psize_block; p_i ++) {
                ...
                for(unsigned i = threadIdx.y; i < DEGREE; i += blockDim.y){
                    ...
                    for(unsigned j = laneid; j < (DIM / 4); j += blockDim.x){
                        ...
                        half4 val2 ; 
                        val2.x = __half2(
                            __float2half((float) batch_values[vec_offset + static_cast<size_t>(j * 4)] / 100.0) ,
                            __float2half((float) batch_values[vec_offset + static_cast<size_t>(j * 4 + 1)] / 100.0)
                        ) ;
                        val2.y = __half2(
                            __float2half((float) batch_values[vec_offset + static_cast<size_t>(j * 4 + 2)] / 100.0) ,
                            __float2half((float) batch_values[vec_offset + static_cast<size_t>(j * 4 + 3)] / 100.0) 
                        ) ;
                        val_res = __hfma2(__hsub2(tmp_val_sha[j].x, val2.x), __hsub2(tmp_val_sha[j].x, val2.x), val_res);
                        val_res = __hfma2(__hsub2(tmp_val_sha[j].y, val2.y), __hsub2(tmp_val_sha[j].y, val2.y), val_res);
                    }
                    ...
                    for(unsigned j = threadIdx.y; j < DEGREE; j += blockDim.y){
                        ...
                        if(top_M_Cand[TOPM + j] == 0xFFFFFFFFu)
                            ...
                            for(unsigned k = laneid; k < (DIM / 4); k += blockDim.x){
                                ...
                                half4 val2 ; 
                                val2.x = __half2(
                                    __float2half((float) batch_values[vec_offset + static_cast<size_t>(k * 4)] / 100.0) ,
                                    __float2half((float) batch_values[vec_offset + static_cast<size_t>(k * 4 + 1)] / 100.0)
                                ) ;
                                val2.y = __half2(
                                    __float2half((float) batch_values[vec_offset + static_cast<size_t>(k * 4 + 2)] / 100.0) ,
                                    __float2half((float) batch_values[vec_offset + static_cast<size_t>(k * 4 + 3)] / 100.0) 
                                ) ;
                                val_res = __hfma2(__hsub2(tmp_val_sha[k].x, val2.x), __hsub2(tmp_val_sha[k].x, val2.x), val_res);
                                val_res = __hfma2(__hsub2(tmp_val_sha[k].y, val2.y), __hsub2(tmp_val_sha[k].y, val2.y), val_res);
                            }
    ...
    template<typename attrType = float , typename vecType = uint8_t>
    __global__ void select_path_contain_attr_filter_V2_without_output_single_channel_with_prefilter_batch_process_compressed_graph_T_dataset_resident_u8(const unsigned batch_size , const unsigned* batch_partition_ids , const unsigned* __restrict__ batch_all_graphs, const vecType* __restrict__ batch_values, const vecType* __restrict__ query_data, 
        unsigned* __restrict__ top_M_Cand_video_memory , float* __restrict__ top_M_Cand_dis_video_memory, unsigned* ent_pts , unsigned DEGREE , unsigned TOPM , unsigned DIM , const unsigned* batch_graph_start_index ,const unsigned* batch_graph_size ,
        const unsigned* __restrict__ batch_pids , const unsigned* batch_p_size ,const unsigned* batch_p_start_index ,const attrType* l_bound ,const attrType* r_bound ,const attrType* __restrict__ attrs , unsigned attr_dim , const unsigned* __restrict__ batch_global_graph , unsigned edges_num_in_global_graph,
        unsigned recycle_list_size ,const unsigned* __restrict__ global_idx ,const unsigned* __restrict__ local_idx ,const unsigned* seg_global_idx_start_idx , unsigned pnum ,unsigned K, unsigned* result_k_id  ,const unsigned* task_id_mapper , const unsigned* batch_little_seg_counts
        ,const unsigned* global_vec_stored_pos , const unsigned batch_all_point_num , const unsigned OP_TYPE = 0) {
    ...
    vector<vector<unsigned>> batch_graph_search_gpu_with_prefilter_batch_process_compressed_graph_T_dataset_resident(unsigned batch_num , vector<vector<unsigned>>& batch_info , unsigned degree , const float* data_ori  , const int8_t* data_ , const float* scales , const int* zero_points , array<unsigned*,2>& graph_buffer , array<unsigned*,2>& global_graph_buffer ,
        ...
        vector<vector<unsigned>> results(qnum , vector<unsigned>(K , 0xFFFFFFFFu)) ;
        vector<vector<float>> results_vec_dis(qnum , vector<float>(K , INF_DIS)) ;
        cout << "dim : " << dim << ", L : " << L << ", DEGREE : " << degree << endl ;
        unsigned DIM = dim , TOPM = L ,  DEGREE = degree ;
        unsigned *results_id[2] , * results_host[2] ;
        float *results_dis[2] ; 
        unsigned *result_k_id ; 
        constexpr unsigned POST_THREAD_NUM = 9 ;
        std::thread post_threads[2][POST_THREAD_NUM] ;


        // ****************************************************************************
        auto join_turn_threads = [&](unsigned turn) {
            for (unsigned t = 0; t < POST_THREAD_NUM; t++) {
                if (post_threads[turn][t].joinable()) {
                    post_threads[turn][t].join();
                }
            }
        };
        auto launch_post_threads = [&](unsigned turn) {
            unsigned chunk = (qnum + POST_THREAD_NUM - 1) / POST_THREAD_NUM;
        
            for (unsigned t = 0; t < POST_THREAD_NUM; t++) {
                unsigned q_begin = t * chunk;
                unsigned q_end = std::min(qnum, (t + 1) * chunk);
        
                post_threads[turn][t] = std::thread([&, turn, q_begin, q_end]() {
                    if (q_begin >= q_end) return;
        
                    unsigned* buf = results_host[turn];
        
                    for (unsigned qid = q_begin; qid < q_end; qid++) {
                        unsigned* src_begin = buf + static_cast<size_t>(qid) * L;
                        unsigned* src_end   = src_begin + L;
        
                        // 这里就是你的后处理逻辑
                        vector<float> tmp_dis(L) ;
                        vector<int> args_indices(L) ;
                        iota(args_indices.begin() , args_indices.end() , 0) ;
                        // 距离计算
                        for(int v = 0 ; v < L ; v ++) {
                            unsigned vid = *(src_begin + v) ;
                            if(vid >= 100000000) {
                                tmp_dis[v] = INF_DIS ;
                                continue ;
                            }
                            ...
                            for(int d = 0 ; d < dim ; d ++) {
                                ...
                                dis += (data_ori[v_index] - query_ori[qid * dim + d]) * (data_ori[v_index] - query_ori[qid * dim + d]) ;
                            }
                            tmp_dis[v] = dis ;
                        }
                        sort(args_indices.begin() , args_indices.end() , [&] (int a , int b) {
                            return tmp_dis[a] < tmp_dis[b] ;
                        }) ;

                        vector<unsigned> t_result = results[qid] ;
                        vector<float> t_result_dis = results_vec_dis[qid] ;
                        int i_index = 0 , j_index = 0 , tar_index = 0 ;
                        while(tar_index < K) {
                            if(t_result_dis[i_index] < tmp_dis[args_indices[j_index]])  {
                                results[qid][tar_index] = t_result[i_index] ;
                                results_vec_dis[qid][tar_index ++] = t_result_dis[i_index ++] ;
                            } else {
                                results[qid][tar_index] = *(src_begin + args_indices[j_index]) ;
                                results_vec_dis[qid][tar_index ++] = tmp_dis[args_indices[j_index ++]] ;
                            }
                        }
                    }
                });
        ...
        cudaMalloc((void**) &results_id[0] , qnum * L * sizeof(unsigned)) ;
        cudaMalloc((void**) &results_id[1] , qnum * L * sizeof(unsigned)) ;
        cudaMalloc((void**) &results_dis[0] , qnum * L * sizeof(float)) ;
        cudaMalloc((void**) &results_dis[1] , qnum * L * sizeof(float)) ;
        cudaMallocHost((void**) &results_host[0] , qnum * L * sizeof(unsigned)) ;
        cudaMallocHost((void**) &results_host[1] , qnum * L * sizeof(unsigned)) ;
        cudaMalloc((void**) &result_k_id , qnum * K * sizeof(unsigned)) ;
        cudaMalloc((void**) &task_id_mapper , qnum * sizeof(unsigned)) ;
        thrust::fill(thrust::device , results_id[0] , results_id[0] + qnum * L , 0xFFFFFFFF) ;
        thrust::fill(thrust::device , results_dis[0] , results_dis[0] + qnum * L , INF_DIS) ;
        thrust::fill(thrust::device , results_id[1] , results_id[1] + qnum * L , 0xFFFFFFFF) ;
        thrust::fill(thrust::device , results_dis[1] , results_dis[1] + qnum * L , INF_DIS) ;
    
        array<array<unsigned* , 4> , 2> partition_buffer ; 
        ...
        cudaMalloc((void**) &partition_buffer[0][3] , qnum * sizeof(unsigned)) ;
        cudaMalloc((void**) &partition_buffer[1][3] , qnum * sizeof(unsigned)) ;
        ...
        **/
        ...
        for(unsigned batch = 0 ; batch < batch_num ; batch ++) {
            ...
            **/
            ...
            fill_partition_info03<<<(qnum + 31) / 32 , 32 ,0 ,  stream_non_block>>>(batch_size , batch_partition_ids_address_pointer , partition_buffer[turn][0] ,partition_buffer[turn][1] ,partition_buffer[turn][2] , partition_buffer[turn][3] ,
                partition_is_selected , qnum , pnum , (bool*) partition_infos[4]) ;


            if(batch > 0) {
                // 若不是第一批, 说明当前有结果需要处理
                cudaStreamSynchronize(0) ;
                // 0号流同步完成后, 结果存放在了results_id[turn^1]
                join_turn_threads(turn ^ 1) ;
                // 发射9个线程处理
                launch_post_threads(turn ^ 1) ;
            }
            ...
            select_path_contain_attr_filter_V2_without_output_single_channel_with_prefilter_batch_process_compressed_graph_T_dataset_resident<attrType><<<grid_s , block_s , byteSize>>>(batch_size , batch_partition_ids_address_pointer , graph_buffer[turn], data_ , query, 
                scales , zero_points ,
                results_id[turn] , results_dis[turn] , ent_pts , DEGREE , TOPM , DIM , batch_global_graph_start_index_address_pointer , batch_graph_size_address_pointer ,
                ...
                recycle_list_size , idx_mapper[0] , idx_mapper[1] , idx_mapper[2] , pnum , K, result_k_id  , task_id_mapper , partition_buffer[turn][3]
                , idx_mapper[3] , mapper_start) ;
            cudaMemcpyAsync(results_host[turn] , results_id[turn] , qnum * L * sizeof(unsigned) , cudaMemcpyDeviceToHost,  0) ;
        ...
        cudaFree(results_id[0]) ;
        cudaFree(results_id[1]) ;
        cudaFree(results_dis[0]) ;
        cudaFree(results_dis[1]) ;
        cudaFree(results_host[0]) ;
        cudaFree(results_host[1]) ;
        cudaFree(ent_pts) ;
    ...
    template<typename attrType = float>
    vector<vector<unsigned>> batch_graph_search_gpu_with_prefilter_batch_process_compressed_graph_T_dataset_resident_u8(unsigned batch_num , vector<vector<unsigned>>& batch_info , unsigned degree , const float* data_ori , const uint8_t* data_ , array<unsigned*,2>& graph_buffer , array<unsigned*,2>& global_graph_buffer ,
        array<unsigned*,2>& generic_buffer , const float* query_ori , const uint8_t* query , unsigned dim , unsigned qnum ,  attrType* l_bound , attrType* r_bound , attrType* attrs , 
        unsigned attr_dim  ,array<unsigned*,3>& graph_info , array<unsigned*,4>& idx_mapper , vector<unsigned>& seg_partition_point_start_index  , array<unsigned* , 5> partition_infos , unsigned L , unsigned K , 
        unsigned* global_edges , unsigned edge_num_contain , unsigned pnum , bool *partition_is_selected , unsigned recycle_list_size , const unsigned cardinality , unsigned MAX_BATCH_SIZE = 16) {
        // ******************* 传输进来的数据都当成主机端数据
        // global_idx[subgraph_offset[subgraph_id] + local_id] = global_id 
        ...
        **/
        ...
        for(unsigned batch = 0 ; batch < batch_num ; batch ++) {
            ...
            **/
            ...
            select_path_contain_attr_filter_V2_without_output_single_channel_with_prefilter_batch_process_compressed_graph_T_dataset_resident_u8<attrType><<<grid_s , block_s , byteSize>>>(batch_size , batch_partition_ids_address_pointer , graph_buffer[turn], data_ , query, 
                results_id[turn] , results_dis[turn] , ent_pts , DEGREE , TOPM , DIM , batch_global_graph_start_index_address_pointer , batch_graph_size_address_pointer ,
                partition_buffer[turn][0] , partition_buffer[turn][1] , partition_buffer[turn][2] , l_bound , r_bound , attrs , attr_dim , global_graph_buffer[turn] , edge_num_contain ,
                recycle_list_size , idx_mapper[0] , idx_mapper[1] , idx_mapper[2] , pnum , K, result_k_id  , task_id_mapper , partition_buffer[turn][3]
                , idx_mapper[3] , mapper_start) ;
    ...
    template<typename attrType = float>
    vector<vector<unsigned>> batch_graph_search_gpu_with_prefilter_batch_process_compressed_graph_T_dataset_resident_straight(unsigned batch_num , vector<vector<unsigned>>& batch_info , unsigned degree , const float* data_ori  , const int8_t* data_ , const float* scales , const int* zero_points , array<unsigned*,2>& graph_buffer , array<unsigned*,2>& global_graph_buffer ,
        array<unsigned*,2>& generic_buffer , const float* query_ori , __half* query , unsigned dim , unsigned qnum ,  attrType* l_bound , attrType* r_bound , attrType* attrs , 
        unsigned attr_dim  ,array<unsigned*,3>& graph_info , array<unsigned*,4>& idx_mapper , vector<unsigned>& seg_partition_point_start_index  , array<unsigned* , 5> partition_infos , unsigned L , unsigned K , 
        unsigned* global_edges , unsigned edge_num_contain , unsigned pnum , bool *partition_is_selected , unsigned recycle_list_size , const unsigned cardinality , unsigned MAX_BATCH_SIZE = 16) {
        // ******************* 传输进来的数据都当成主机端数据
        ...
        for(unsigned batch = 0 ; batch < batch_num ; batch ++) {
            ...
            select_path_contain_attr_filter_V2_without_output_single_channel_with_prefilter_batch_process_compressed_graph_T_dataset_resident_straight<attrType><<<grid_s , block_s , byteSize>>>(batch_size , batch_partition_ids_address_pointer , graph_buffer[turn], data_ , query, 
                scales , zero_points ,
                results_id[turn] , results_dis[turn] , ent_pts , DEGREE , TOPM , DIM , batch_global_graph_start_index_address_pointer , batch_graph_size_address_pointer ,
                partition_buffer[turn][0] , partition_buffer[turn][1] , partition_buffer[turn][2] , l_bound , r_bound , attrs , attr_dim , global_graph_buffer[turn] , edge_num_contain ,
    ...
    template<typename attrType = float , typename vecType = uint8_t>
    vector<vector<unsigned>> batch_graph_search_gpu_with_prefilter_batch_process_compressed_graph_T_dataset_resident_straight_u8(unsigned batch_num , vector<vector<unsigned>>& batch_info , unsigned degree  , const vecType* data_ , array<unsigned*,2>& graph_buffer , array<unsigned*,2>& global_graph_buffer ,
        array<unsigned*,2>& generic_buffer , const vecType* query , unsigned dim , unsigned qnum ,  attrType* l_bound , attrType* r_bound , attrType* attrs , 
        unsigned attr_dim  ,array<unsigned*,3>& graph_info , array<unsigned*,4>& idx_mapper , vector<unsigned>& seg_partition_point_start_index  , array<unsigned* , 5> partition_infos , unsigned L , unsigned K , 
        unsigned* global_edges , unsigned edge_num_contain , unsigned pnum , bool *partition_is_selected , unsigned recycle_list_size , const unsigned cardinality , unsigned MAX_BATCH_SIZE = 16) {
        // ******************* 传输进来的数据都当成主机端数据
        // global_idx[subgraph_offset[subgraph_id] + local_id] = global_id 
        ...
        **/
        ...
        for(unsigned batch = 0 ; batch < batch_num ; batch ++) {
            ...
            select_path_contain_attr_filter_V2_without_output_single_channel_with_prefilter_batch_process_compressed_graph_T_dataset_resident_straight_u8<attrType , vecType><<<grid_s , block_s , byteSize>>>(batch_size , batch_partition_ids_address_pointer , graph_buffer[turn], data_ , query, 
